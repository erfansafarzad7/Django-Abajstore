...
def generate_otp(phone_number):
    ...
    OTP.objects.update_or_create(phone_number=phone_number, defaults={'code': code, })
    return code
...
def send_otp(phone_number, reason):
    ...
    if reason == 'register':
        print('send sms : ', phone_number, ' ', code)
...
def create_or_update_user(request):
    ...
    # ======================= Update User ========================
        ...
        if reason == 'set_new_password':
            ...
            try:
                # ==================== send sms ==========================
                print('send sms : ', phone_number, ' ', new_password)
                # =========================================================
...
class VerifyView(FormView):
    ...
    def form_valid(self, form):
        ...
        if form.is_valid() and session:
            ...
            otp = OTP.objects.filter(phone_number__exact=session['phone_number'])

            if otp and otp.filter(code__exact=user_entered_code).exists():
                otp.delete()
